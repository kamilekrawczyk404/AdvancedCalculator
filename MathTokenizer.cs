...
namespace _13032025
{
    class MathTokenizer
    {
        public static string[] FormatConstants(string value, double toInsert)
        {
            ...
            return formatted.Contains(" ") ? formatted.Split(" ") : [formatted];
        }

        public static string FormatNegativeValues(string value)
        {
        ...
        public static List<string> Tokenize(string expression)
        {
            ...
            //string pattern = @"(?<!\d)-?\d+(\,\d+)?([eE][+-]?\d+)?|π|e|%|[+÷\-⨯^%!()]|√|\w+";
            string pattern = @"-\(\d+(?:,\d+)?\)|-?\d*(?:,\d+)?(?:[eE][+-]?\d+)?π|-?\d*(?:,\d+)?(?:[eE][+-]?\d+)?e|-?\d+(?:,\d+)?(?:[eE][+-]?\d+)?|¯¹|[+÷\-⨯^%!²()]|√|(?:sin|cos|tan|log|ln)";
            ...
            foreach (Match match in Regex.Matches(expression, pattern))
            {
                ...
                else if (match.Value.Contains("π"))
                {
                    ...
                    tokens.Add(MathTokenizer.FormatNegativeValues(match.Value));
                }
                else
                {
